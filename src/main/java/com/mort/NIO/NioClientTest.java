...
public class NioClientTest {
    ...
    protected void sendMessages(InetAddress host, boolean direct, int num_threads) throws Exception {
        ...
        senders=new NioClientTest.Sender[num_threads];
        final CountDownLatch latch=new CountDownLatch(1);
        for(int i=0; i < senders.length; i++)
            senders[i]=new NioClientTest.Sender(host, direct, latch);
        for(NioClientTest.Sender sender: senders)
            sender.start();
        latch.countDown();
        for(NioClientTest.Sender sender: senders)
            sender.join();
